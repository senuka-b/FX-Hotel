...
import edu.icet.senuka.FXHotelManager.util.OTPEmailThread;
...
public class ForgotPasswordDialogFormController extends SuperController {
    ...
    @FXML
    void buttonResetPasswordOnAction(ActionEvent event) {
    ...
    @FXML
    void buttonSendOTPOnAction(ActionEvent event) throws AddressException {
        ...
        for (User user : users) {
            if (user.getEmail().equals(textFieldEmail.getText().trim().toLowerCase())) {
                otp = generateOTP();
                ...
                OTPEmailThread.sendEmail(content, textFieldEmail.getText().trim(), data -> {
                    Platform.runLater(() -> {
                        showAlert(
                                Alert.AlertType.INFORMATION,
                                "OTP Verification",
    ...
    @FXML
    void textFieldOTPKeyOnKeyReleased(KeyEvent event) {
